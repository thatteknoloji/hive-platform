---
title: "0-Day Exploit Avcısı"
slug: "zeroday"
category: "Modül Ansiklopedisi"
level: "Reference"
order: "0"
status: "auto-draft"
version: "1.0.0"
last_updated: "2026-06-26"
owner: "HIVE Team"
related:
  - "⚔️ Müdahale & Saldır"
---

# 0-Day Exploit Avcısı

## Bu bölümde ne öğreneceksiniz?

**0-Day Exploit Avcısı** modülünün HIVE içindeki rolünü özet düzeyde öğreneceksiniz.

## Kısa cevap

WordPress plugin zafiyet taraması

## Detaylı açıklama

**Modül ID:** `zeroday`

**Grup:** ⚔️ Müdahale & Saldırı

**API endpoint:** `/api/zeroday'`

<!-- AI_UPDATE_SLOT: module_zeroday_detail -->

## HIVE içinde nerede kullanılır?

Panel: `/api/zeroday` veya klasik modül listesinden **0-Day Exploit Avcısı**.

## Adım adım kullanım

1. Aktif projeyi seçin.
2. Modül sayfasını açın.
3. Gerekli alanları doldurun.
4. Çalıştır ve sonucu Brain'de kontrol edin.

<!-- TODO: modüle özel adımlar -->

## Örnek senaryo

<!-- TODO: örnek senaryo -->

## Görsel alanı

> 📷 Screenshot placeholder: 0-Day Exploit Avcısı modül ekranı.

## Akış diyagramı

```mermaid
flowchart TD
    A[Aktif Proje] --> B[Modül]
    B --> C[Sonuç / Log]
```

## En iyi kullanım

<!-- TODO: en iyi kullanım -->

## Yapılmaması gerekenler

<!-- TODO: yapılmaması gerekenler -->

## Yaygın hatalar

<!-- TODO: yaygın hatalar -->

## Sık sorulan sorular

**Bu modül içerik üretir mi?** Ansiklopedi taslağı — detay için Mission Control ve modül health endpoint'ini kontrol edin.

<!-- EKSİK_BİLGİ: zeroday derin dokümantasyon -->

## İlgili konular

- Grup: ⚔️ Müdahale & Saldırı
- [Modül Ansiklopedisi README](../06-modul-ansiklopedisi/README.md)

## Sonraki konu

İlgili modül gruplarına göre Campaign Engine veya Mission Control'a bakın.
