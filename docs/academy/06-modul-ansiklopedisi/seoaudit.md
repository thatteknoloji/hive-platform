---
title: "SEO Audit"
slug: "seoaudit"
category: "Modül Ansiklopedisi"
level: "Reference"
order: "0"
status: "auto-draft"
version: "1.0.0"
last_updated: "2026-06-26"
owner: "HIVE Team"
related:
  - "Keşif & Analiz"
---

# SEO Audit

## Bu bölümde ne öğreneceksiniz?

**SEO Audit** modülünün HIVE içindeki rolünü özet düzeyde öğreneceksiniz.

## Kısa cevap

Site sağlık kontrolü ve hata raporu

## Detaylı açıklama

**Modül ID:** `seoaudit`

**Grup:** 🔍 Keşif & Analiz

**API endpoint:** `/api/seoaudit'`

<!-- AI_UPDATE_SLOT: module_seoaudit_detail -->

## HIVE içinde nerede kullanılır?

Panel: `/api/seoaudit` veya klasik modül listesinden **SEO Audit**.

## Adım adım kullanım

1. Aktif projeyi seçin.
2. Modül sayfasını açın.
3. Gerekli alanları doldurun.
4. Çalıştır ve sonucu Brain'de kontrol edin.

<!-- TODO: modüle özel adımlar -->

## Örnek senaryo

<!-- TODO: örnek senaryo -->

## Görsel alanı

> 📷 Screenshot placeholder: SEO Audit modül ekranı.

## Akış diyagramı

```mermaid
flowchart TD
    A[Aktif Proje] --> B[Modül]
    B --> C[Sonuç / Log]
```

## En iyi kullanım

<!-- TODO: en iyi kullanım -->

## Yapılmaması gerekenler

<!-- TODO: yapılmaması gerekenler -->

## Yaygın hatalar

<!-- TODO: yaygın hatalar -->

## Sık sorulan sorular

**Bu modül içerik üretir mi?** Ansiklopedi taslağı — detay için Mission Control ve modül health endpoint'ini kontrol edin.

<!-- EKSİK_BİLGİ: seoaudit derin dokümantasyon -->

## İlgili konular

- Grup: 🔍 Keşif & Analiz
- [Modül Ansiklopedisi README](../06-modul-ansiklopedisi/README.md)

## Sonraki konu

İlgili modül gruplarına göre Campaign Engine veya Mission Control'a bakın.
