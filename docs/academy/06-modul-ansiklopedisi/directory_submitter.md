---
title: "Directory Submitter"
slug: "directory_submitter"
category: "Modül Ansiklopedisi"
level: "Reference"
order: "0"
status: "auto-draft"
version: "1.0.0"
last_updated: "2026-06-26"
owner: "HIVE Team"
related:
  - "📝 İçerik & SEO"
---

# Directory Submitter

## Bu bölümde ne öğreneceksiniz?

**Directory Submitter** modülünün HIVE içindeki rolünü özet düzeyde öğreneceksiniz.

## Kısa cevap

120+ dizine toplu gönderim

## Detaylı açıklama

**Modül ID:** `directory_submitter`

**Grup:** 📝 İçerik & SEO

**API endpoint:** `/api/directory_submitter'`

<!-- AI_UPDATE_SLOT: module_directory_submitter_detail -->

## HIVE içinde nerede kullanılır?

Panel: `/api/directory_submitter` veya klasik modül listesinden **Directory Submitter**.

## Adım adım kullanım

1. Aktif projeyi seçin.
2. Modül sayfasını açın.
3. Gerekli alanları doldurun.
4. Çalıştır ve sonucu Brain'de kontrol edin.

<!-- TODO: modüle özel adımlar -->

## Örnek senaryo

<!-- TODO: örnek senaryo -->

## Görsel alanı

> 📷 Screenshot placeholder: Directory Submitter modül ekranı.

## Akış diyagramı

```mermaid
flowchart TD
    A[Aktif Proje] --> B[Modül]
    B --> C[Sonuç / Log]
```

## En iyi kullanım

<!-- TODO: en iyi kullanım -->

## Yapılmaması gerekenler

<!-- TODO: yapılmaması gerekenler -->

## Yaygın hatalar

<!-- TODO: yaygın hatalar -->

## Sık sorulan sorular

**Bu modül içerik üretir mi?** Ansiklopedi taslağı — detay için Mission Control ve modül health endpoint'ini kontrol edin.

<!-- EKSİK_BİLGİ: directory_submitter derin dokümantasyon -->

## İlgili konular

- Grup: 📝 İçerik & SEO
- [Modül Ansiklopedisi README](../06-modul-ansiklopedisi/README.md)

## Sonraki konu

İlgili modül gruplarına göre Campaign Engine veya Mission Control'a bakın.
