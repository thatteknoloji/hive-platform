---
title: "AI Citation Tracker"
slug: "ai_citation"
category: "Modül Ansiklopedisi"
level: "Reference"
order: "0"
status: "auto-draft"
version: "1.0.0"
last_updated: "2026-06-26"
owner: "HIVE Team"
related:
  - "🤖 AI & Veri"
---

# AI Citation Tracker

## Bu bölümde ne öğreneceksiniz?

**AI Citation Tracker** modülünün HIVE içindeki rolünü özet düzeyde öğreneceksiniz.

## Kısa cevap

AI kaynaklarında marka takibi

## Detaylı açıklama

**Modül ID:** `ai_citation`

**Grup:** 🤖 AI & Veri

**API endpoint:** `/api/ai_citation'`

<!-- AI_UPDATE_SLOT: module_ai_citation_detail -->

## HIVE içinde nerede kullanılır?

Panel: `/api/ai_citation` veya klasik modül listesinden **AI Citation Tracker**.

## Adım adım kullanım

1. Aktif projeyi seçin.
2. Modül sayfasını açın.
3. Gerekli alanları doldurun.
4. Çalıştır ve sonucu Brain'de kontrol edin.

<!-- TODO: modüle özel adımlar -->

## Örnek senaryo

<!-- TODO: örnek senaryo -->

## Görsel alanı

> 📷 Screenshot placeholder: AI Citation Tracker modül ekranı.

## Akış diyagramı

```mermaid
flowchart TD
    A[Aktif Proje] --> B[Modül]
    B --> C[Sonuç / Log]
```

## En iyi kullanım

<!-- TODO: en iyi kullanım -->

## Yapılmaması gerekenler

<!-- TODO: yapılmaması gerekenler -->

## Yaygın hatalar

<!-- TODO: yaygın hatalar -->

## Sık sorulan sorular

**Bu modül içerik üretir mi?** Ansiklopedi taslağı — detay için Mission Control ve modül health endpoint'ini kontrol edin.

<!-- EKSİK_BİLGİ: ai_citation derin dokümantasyon -->

## İlgili konular

- Grup: 🤖 AI & Veri
- [Modül Ansiklopedisi README](../06-modul-ansiklopedisi/README.md)

## Sonraki konu

İlgili modül gruplarına göre Campaign Engine veya Mission Control'a bakın.
