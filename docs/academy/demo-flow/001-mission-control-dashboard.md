---
title: "Mission Control — Dashboard"
slug: "mission_control_center"
category: "Demo Flow"
level: "Beginner"
order: "1"
status: "published"
version: "1.1.0"
last_updated: "2026-06-26"
last_editor: "HIVE Team"
owner: "HIVE Team"
difficulty: "Başlangıç"
reading_time_minutes: "10"
related_modules:
  - "mission_control_center"
  - "projects"
  - "campaign_engine"
related_api:
  - "GET /api/mission-control/dashboard"
  - "GET /api/mission-control/dashboard-full"
  - "POST /api/mission-control/action/{id}/ack"
prerequisites:
  - "Aktif proje seçili"
learn_items:
  - "War Room dashboard"
  - "Next best actions"
  - "Canlı metrikler"
badge_reward: "first_dashboard"
---

# Mission Control — Dashboard

## Bu bölümde ne öğreneceksiniz?

HIVE ana dashboard'unu (War Room / Mission Control) ve demo sırasında nasıl sunulacağını öğreneceksiniz.

## Kısa cevap

**Mission Control** CEO cockpit'tir: kampanya durumu, fırsatlar, authority, provider alarmları ve önerilen aksiyonlar tek ekranda toplanır. Aktif proje zorunludur.

## Detaylı açıklama

**Modül ID:** `mission_control_center`  
**Frontend:** `MissionControlCenter.js` + `HiveWarRoom`  
**Backend:** `/api/mission-control/*`

Dashboard iki modda yüklenir:
- `GET /api/mission-control/dashboard` — hızlı özet
- `GET /api/mission-control/dashboard-full` — tam War Room verisi

90 saniyede bir sessiz yenileme yapılır (sekme görünürken).

## HIVE içinde nerede kullanılır?

COMMAND → **Mission Control** — panel açılış ekranı (varsayılan)

## Adım adım kullanım (Demo)

1. Üst bardan **aktif proje** seçin.
2. Mission Control açılır — metrik kartları yüklenir.
3. **Next Best Actions** panelinden önerilen adımları inceleyin.
4. Bir aksiyonu **Ack** veya **Done** ile işaretleyin.
5. İlgili modüle geçiş için kartlara tıklayın.

## Örnek senaryo

Müşteri demo: "Tek ekrandan tüm SEO operasyonunu görüyorum" — War Room'da kampanya, keyword fırsatı ve publisher durumu yan yana gösterilir.

## Görsel alanı

> 📷 Screenshot: War Room tam dashboard — metrik grid + next actions.

## Akış diyagramı

```mermaid
flowchart TD
    P[Aktif Proje] --> API[/api/mission-control/dashboard/]
    API --> WR[HiveWarRoom UI]
    WR --> A1[Campaign Engine]
    WR --> A2[Publisher Hub]
    WR --> A3[Authority Mesh]
    WR --> NBA[Next Best Actions]
```

## API

| Method | Endpoint | Açıklama |
|--------|----------|----------|
| GET | `/api/mission-control/dashboard` | Özet dashboard |
| GET | `/api/mission-control/dashboard-full` | Tam War Room |
| POST | `/api/mission-control/action/{id}/ack` | Aksiyon onayla |
| POST | `/api/mission-control/action/{id}/done` | Aksiyon tamamla |
| GET | `/api/mission-control/health` | Health check |

## En iyi kullanım

- Demo öncesi aktif projede en az bir kampanya veya keyword verisi olsun.
- Boş dashboard'da önce Projects → demo proje oluşturun.

## Yapılmaması gerekenler

- Aktif proje olmadan dashboard açmayı müşteriye "hata" gibi göstermeyin — bilinçli empty state'tir.

## Yaygın hatalar / Troubleshooting

| Sorun | Çözüm |
|-------|--------|
| "Aktif proje yok" | Projects → proje seç → üst bar |
| Dashboard boş | Demo verisi yok — wizard veya seed çalıştırın |
| 500 hata | `/api/mission-control/health` kontrol |
| Yavaş yükleme | Önce özet dashboard, sonra "Load Full" |

## Sık sorulan sorular

**Polling kapatılır mı?** Sekme gizliyken istek atlanır; performans için tasarlandı.

## İlgili konular

- [Active Project Context](../02-firma-proje-yonetimi/002-active-project-context)
- [Users & Permissions](../02-firma-proje-yonetimi/003-users-permissions)
- [Campaign Engine](../06-modul-ansiklopedisi/campaign_engine)

## Sonraki konu

GROUP B: **Talon** ve **Quality Gate**

<!-- AI_UPDATE_SLOT -->
